...
# Create a state token to prevent request forgrey.
...
# Endpoint for login with google accounts
...
def newCategory():
    ...
    if request.method == 'POST':
        if request.form['name']:
            try:
                isExist = session.query(Category).filter_by(name=request.
                                                            form['name']).one()
            except:
...
def showItem(category_id):
    ...
    if 'username' not in login_session:
        return render_template('publicItem.html', items=items,
                               categories=categories, categoryOne=categoryOne,
                               creator=creator)
    elif creator.id != login_session['user_id']:
        return render_template('publicItemUser.html', items=items,
                               categories=categories, categoryOne=categoryOne,
                               creator=creator)
    else:
...
def newItem(category_id):
    ...
    if request.method == 'POST':
        if request.form['name']:
            newItem = Item(name=request.form['name'], description=request.
                           form['description'], category_id=category_id,
...
# Create a user by user login session
